...
export default function UserVisitsPage() {
  ...
  useEffect(() => {
    ...
    const FetchUserAppointments = async () => {
      const url = `/api/fetch_user_appointments/?current_user=${current_user}&timeframe=previous`;
      ...
      try {
        ...
        if (response.ok) {
          ...
          console.log('Success in fetching the user visits');
        } else {
          console.log('Could not fetch the user visits');
        }
      } catch (error) {
        console.log(`Error in fetching the user visits: ${error}`);
      }
    };
  ...
  const headings_names = ['Doctor', 'Patient', 'Visit'];
  ...
  return (
    <div className="main-container">
      ...
      <div className="appointments-main-container">
        ...
        <div>
          <i className="fa-solid fa-notes-medical inner-icons"></i>
        </div>
        <div>User Visits</div>
        
      
      <div className="instruction-messages">
          Your Visits in Grow Well Hospitals.
        </div>
        </div>
